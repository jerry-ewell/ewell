...
import { getContractMethods, transformArrayToMap, getTxResult, messageHTML } from './aelfUtils';
...
export class AElfContractBasic {
  ...
  public callSendMethod: AElfCallSendMethod = async (functionName, paramsOption) => {
    ...
    try {
      ...
      await sleep(1000);
      try {
        return await getTxResult(TransactionId);
      } catch (error: any) {
        messageHTML(TransactionId, 'error');
        return { error };
      }
    } catch (e: any) {
